...
class main_model extends CI_Model {
    ...
    public function detail_kendala($id_kendala, $kendala) {
        ...
        $this->db->from('TX_REAL_SUB_PROGRAMm a');
        $this->db->join('TM_STATUS_PROGRAM b', 'a.REAL_SUBPRO_STATUS = b.STATUS_ID');
        $this->db->join('TM_CONTRAINTS c', 'a.REAL_SUBPRO_CONSTRAINTS = c.CONTRAINTS_ID');
        $this->db->join('TX_RKAP_SUB_PROGRAM d', 'a.RKAP_SUBPRO_ID = d.RKAP_SUBPRO_ID');
        $this->db->join('TR_SUBPRO_TYPE e', 'd.RKAP_SUBPRO_TYPE_ID = e.SUBPRO_TYPE_ID');
    ...
    public function kpi_realisasi_fisik($id_branch, $get_bulan, $get_tahun) {
        $query = $this->db->query("SELECT A.BRANCH_ID, (A.REAL_SUBPRO_VAL / B.TOTAL_NILAI_CONTRACT * 100) AS KPI_REALISASI_FISIK FROM
        (
            SELECT BRANCH_ID, SUM(REAL_SUBPRO_VAL) AS REAL_SUBPRO_VAL 
            FROM (SELECT BRANCH_ID, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE 
            FROM (SELECT RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE, BRANCH_ID
   ...
   public function kontrak_kritis($id_branch) {
    ...
    public function pusat_kpi_realisasi_fisik($is_pusat, $get_bulan, $get_tahun) {
        $query = $this->db->query("SELECT A.IS_PUSAT, (A.REAL_SUBPRO_VAL / B.TOTAL_NILAI_CONTRACT * 100) AS KPI_REALISASI_FISIK FROM
        (
            SELECT IS_PUSAT, SUM(REAL_SUBPRO_VAL) AS REAL_SUBPRO_VAL 
            FROM (SELECT IS_PUSAT, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE 
            FROM (SELECT RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE, IS_PUSAT
   ...
   public function pusat_kontrak_kritis($is_pusat) {
    ...
    public function all_kpi_realisasi_fisik($get_bulan, $get_tahun) {
        $query = $this->db->query("SELECT A.COMPANY_CODE, (A.REAL_SUBPRO_VAL / B.TOTAL_NILAI_CONTRACT * 100) AS KPI_REALISASI_FISIK FROM
        (
            SELECT COMPANY_CODE, SUM(REAL_SUBPRO_VAL) AS REAL_SUBPRO_VAL 
            FROM (SELECT COMPANY_CODE, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE 
            FROM (SELECT RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE, COMPANY_CODE
   ...
   public function gauge_kritis_3_p($is_pusat, $get_bulan, $get_kontrak_end) {
    ...
    public function sipil_rkap() {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_PROGRAM FROM
            ...
            (
                SELECT BRANCH_ID, BRANCH_NAME, SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                FROM(SELECT a.RKAP_INVS_ASSETS, COUNT(a.RKAP_INVS_ASSETS) AS JUMLAH_PROGRAM,b.ASSETS_NAME, d.BRANCH_ID, d.BRANCH_NAME
                FROM TX_RKAP_INVESTATION a
                JOIN TM_ASSETS b ON a.RKAP_INVS_ASSETS = b.ASSETS_ID
                JOIN TM_USERS c ON a.RKAP_INVS_USER_ID = c.USER_ID 
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID 
                where a.RKAP_INVS_ASSETS IN ('1', '3', '9', '7')
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
        return $query->result();
    }
    ...
    public function sipil_berjalan() {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_PROGRAM FROM
            ...
            (
                SELECT BRANCH_ID, BRANCH_NAME, SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                FROM(SELECT RKAP_INVS_ASSETS, COUNT(RKAP_INVS_ASSETS) AS JUMLAH_PROGRAM, ASSETS_NAME, BRANCH_NAME, BRANCH_ID
                FROM(SELECT a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE, e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID
                FROM TX_RKAP_INVESTATION a
                JOIN TX_RKAP_SUB_PROGRAM b ON a.RKAP_INVS_ID = b.RKAP_SUBPRO_INVS_ID 
                JOIN TX_REAL_SUB_PROGRAM c ON b.RKAP_SUBPRO_ID = c.RKAP_SUBPRO_ID
                JOIN TM_STATUS_PROGRAM d ON c.REAL_SUBPRO_STATUS = d.STATUS_ID
                JOIN TM_ASSETS e ON a.RKAP_INVS_ASSETS = e.ASSETS_ID
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND a.RKAP_INVS_ASSETS IN ('1', '3', '9', '7')
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
        return $query->result();
    }
    ...
    public function peralatan_rkap() {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_PROGRAM FROM
            ...
            (
                ...
                JOIN TM_USERS c ON a.RKAP_INVS_USER_ID = c.USER_ID 
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID 
                where a.RKAP_INVS_ASSETS IN ('2', '4', '5', '6')
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
        return $query->result();
    ...
    public function peralatan_berjalan() {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_PROGRAM FROM
            ...
            (
                ...
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND a.RKAP_INVS_ASSETS IN ('2', '4', '5', '6')
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
    ...
    public function non_fisik_berjalan() {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_PROGRAM FROM
            ...
            (
                ...
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND a.RKAP_INVS_ASSETS = 8
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
    ...
    public function jumlah_sipil_rkap() {
        $query =$this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
            FROM(SELECT a.RKAP_INVS_ASSETS, COUNT(a.RKAP_INVS_ASSETS) AS JUMLAH_PROGRAM,b.ASSETS_NAME, d.BRANCH_ID, d.BRANCH_NAME
            FROM TX_RKAP_INVESTATION a
            JOIN TM_ASSETS b ON a.RKAP_INVS_ASSETS = b.ASSETS_ID
            JOIN TM_USERS c ON a.RKAP_INVS_USER_ID = c.USER_ID
            JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
            WHERE a.RKAP_INVS_ASSETS IN ('1', '3', '9', '7') AND IS_PUSAT IN ('1', '0')
            GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_ID, BRANCH_NAME)");
        return $query->result();
    }
    ...
    public function jumlah_sipil_berjalan() {
        $query =$this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                ...
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND a.RKAP_INVS_ASSETS IN ('1', '3', '9', '7') AND IS_PUSAT IN ('1', '0')
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)");
        return $query->result();
    }
    ...
    public function jumlah_peralatan_rkap() {
        $query =$this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
            ...
            JOIN TM_USERS c ON a.RKAP_INVS_USER_ID = c.USER_ID
            JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
            WHERE a.RKAP_INVS_ASSETS IN ('2', '4', '5', '6') AND IS_PUSAT IN ('1', '0')
            GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_ID, BRANCH_NAME)");
        return $query->result();
    }
    ...
    public function jumlah_peralatan_berjalan() {
        $query =$this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                ...
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND a.RKAP_INVS_ASSETS IN ('2', '4', '5', '6') AND IS_PUSAT IN ('1', '0')
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)");
        return $query->result();
    }
    ...
    public function jumlah_non_sipil_rkap() {
        $query =$this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
            ...
            JOIN TM_USERS c ON a.RKAP_INVS_USER_ID = c.USER_ID
            JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
            WHERE a.RKAP_INVS_ASSETS = 8 AND IS_PUSAT IN ('1', '0')
            GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_ID, BRANCH_NAME)");
        return $query->result();
    }
    ...
    public function jumlah_non_sipil_berjalan() {
        $query =$this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                ...
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND a.RKAP_INVS_ASSETS = 8 AND IS_PUSAT IN ('1', '0')
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)");
        return $query->result();
    }
    ...
    public function jumlah_total_berjalan() {
        $query = $this->db->query("SELECT SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                ...
                JOIN TM_USERS f ON a.RKAP_INVS_USER_ID = f.USER_ID
                JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                WHERE c.REAL_SUBPRO_STATUS != 2 AND IS_PUSAT IN ('1', '0')
                GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.BRANCH_ID)
                GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, BRANCH_ID)");
        return $query->result();
    }
    ...
    public function persentase_berjalan() {
        $query = $this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.PERSENTASE_BERJALAN FROM
        (
            SELECT * FROM TR_BRANCH
        ) A LEFT JOIN 
        (
            SELECT A.BRANCH_ID, A.BRANCH_NAME, (B.TOTAL_PROGRAM / A.TOTAL_PROGRAM * 100) AS PERSENTASE_BERJALAN FROM
                ( 
                    SELECT BRANCH_ID, BRANCH_NAME, SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                    FROM(SELECT a.RKAP_INVS_ASSETS, COUNT(a.RKAP_INVS_ASSETS) AS JUMLAH_PROGRAM,b.ASSETS_NAME, d.BRANCH_ID, d.BRANCH_NAME
                    FROM TX_RKAP_INVESTATION a
    ...
    public function persentase_berjalan_footer() {
        $query = $this->db->query("            SELECT (TOTAL_2 / TOTAL_1 *100) AS persentase_berjalan
            FROM(SELECT SUM(A.TOTAL_PROGRAM) AS TOTAL_1, SUM(B.TOTAL_PROGRAM) AS TOTAL_2 FROM
                ( 
                    SELECT IS_PUSAT, SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                    FROM(SELECT a.RKAP_INVS_ASSETS, COUNT(a.RKAP_INVS_ASSETS) AS JUMLAH_PROGRAM, b.ASSETS_NAME, d.IS_PUSAT, d.BRANCH_NAME
                    FROM TX_RKAP_INVESTATION a
                    JOIN TM_ASSETS b ON a.RKAP_INVS_ASSETS = b.ASSETS_ID
                    JOIN TM_USERS c ON a.RKAP_INVS_USER_ID = c.USER_ID
                    JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                    where IS_PUSAT IN ('1', '0')
                    GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, IS_PUSAT, BRANCH_NAME)
                    GROUP BY IS_PUSAT
                ) A LEFT JOIN 
                (
                    SELECT IS_PUSAT, SUM(JUMLAH_PROGRAM) AS TOTAL_PROGRAM
                    FROM(SELECT RKAP_INVS_ASSETS, COUNT(RKAP_INVS_ASSETS) AS JUMLAH_PROGRAM, ASSETS_NAME, BRANCH_NAME, IS_PUSAT
                    FROM(SELECT a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE, e.ASSETS_NAME, g.BRANCH_NAME, g.IS_PUSAT
                    FROM TX_RKAP_INVESTATION a
                    JOIN TX_RKAP_SUB_PROGRAM b ON a.RKAP_INVS_ID = b.RKAP_SUBPRO_INVS_ID 
                    JOIN TX_REAL_SUB_PROGRAM c ON b.RKAP_SUBPRO_ID = c.RKAP_SUBPRO_ID
                    JOIN TM_STATUS_PROGRAM d ON c.REAL_SUBPRO_STATUS = d.STATUS_ID
                    ...
                    JOIN TR_BRANCH g ON f.USER_BRANCH = g.BRANCH_ID
                    WHERE c.REAL_SUBPRO_STATUS != 2 AND IS_PUSAT IN ('1', '0')
                    GROUP BY a.RKAP_INVS_ID, a.RKAP_INVS_ASSETS, a.RKAP_INVS_TITLE,e.ASSETS_NAME, g.BRANCH_NAME, g.IS_PUSAT)
                    GROUP BY RKAP_INVS_ASSETS, ASSETS_NAME, BRANCH_NAME, IS_PUSAT)
                    GROUP BY IS_PUSAT
                ) B ON A.IS_PUSAT = B.IS_PUSAT
            )");
        return $query->result();
    ...
    // REKAPITULASI KPI REALISASI

    public function realisasi_sipil_berjalan($get_bulan) {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_REALISASI FROM
            ...
            (
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND c.RKAP_INVS_ASSETS IN ('1', '3', '9', '7'))
                WHERE AA <= '$get_bulan'
    ...
    public function realisasi_peralatan_berjalan($get_bulan) {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_REALISASI FROM
            ...
            (
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND c.RKAP_INVS_ASSETS IN ('2', '4', '5', '6'))
                WHERE AA <= '$get_bulan'
                GROUP BY RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE)
                GROUP BY BRANCH_ID, BRANCH_NAME
    ...
    public function realisasi_non_fisik_berjalan($get_bulan) {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_REALISASI FROM
            ...
            (
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND c.RKAP_INVS_ASSETS = 8)
                WHERE AA <= '$get_bulan'
                GROUP BY RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE)
                GROUP BY BRANCH_ID, BRANCH_NAME
    ...
    public function realisasi_total_report_berjalan($get_bulan) {
        $query = $this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_REALISASI FROM
            ( 
                SELECT * FROM TR_BRANCH
            ) A LEFT JOIN 
    ...
    public function jumlah_realisasi_sipil_berjalan($get_bulan) {
        ...
        $query =$this->db->query("SELECT SUM(REAL_SUBPRO_VAL) AS TOTAL_REALISASI 
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND c.RKAP_INVS_ASSETS IN ('1', '3', '9', '7') AND IS_PUSAT IN ('1', '0'))
                WHERE AA <= '$get_bulan'
                GROUP BY RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE)
                GROUP BY BRANCH_ID, BRANCH_NAME");
    ...
    public function jumlah_realisasi_peralatan_berjalan($get_bulan) {
        ...
        $query =$this->db->query("SELECT SUM(REAL_SUBPRO_VAL) AS TOTAL_REALISASI 
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND c.RKAP_INVS_ASSETS IN ('2', '4', '5', '6') AND IS_PUSAT IN ('1', '0'))
                WHERE AA <= '$get_bulan'
                GROUP BY RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE)");
        return $query->result();
    ...
    public function jumlah_realisasi_non_fisik_berjalan($get_bulan) {
        ...
        $query =$this->db->query("SELECT SUM(REAL_SUBPRO_VAL) AS TOTAL_REALISASI 
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND c.RKAP_INVS_ASSETS = 8 AND IS_PUSAT IN ('1', '0'))
                WHERE AA <= '$get_bulan'
                GROUP BY RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE)");
        return $query->result();
    ...
    public function jumlah_realisasi_total_report_berjalan($get_bulan) {
        $query = $this->db->query("SELECT SUM(REAL_SUBPRO_VAL) AS TOTAL_REALISASI 
                FROM (SELECT BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE 
                FROM (SELECT RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME
                FROM (SELECT a.RKAP_SUBPRO_ID, a.RKAP_SUBPRO_TITTLE, a.RKAP_SUBPRO_INVS_ID, TO_CHAR(b.REAL_SUBPRO_DATE, 'YYYY-MM') as AA, b.REAL_SUBPRO_VAL, b.REAL_SUBPRO_STATUS,
                ...
                JOIN TM_USERS d ON c.RKAP_INVS_USER_ID = d.USER_ID
                JOIN TR_BRANCH e ON d.USER_BRANCH = e.BRANCH_ID
                WHERE a.IS_DELETED = 0 AND b.IS_DELETED = 0 AND c.IS_DELETED = 0 AND b.REAL_SUBPRO_STATUS != 2 AND IS_PUSAT IN ('1', '0'))
                WHERE AA <= '$get_bulan'
                GROUP BY RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME)
                GROUP BY BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, RKAP_INVS_VALUE)");
        return $query->result();
    ...
    public function kontrak_sipil_berjalan($get_tahun) {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_NILAI_CONTRACT FROM
            ( 
                SELECT * FROM TR_BRANCH
            ) A LEFT JOIN 
            (
                SELECT BRANCH_ID , BRANCH_NAME, SUM(RKAP_SUBPRO_CONTRACT_VALUE) AS TOTAL_NILAI_CONTRACT
                FROM (SELECT a.RKAP_SUBPRO_INVS_ID, a.RKAP_SUBPRO_TITTLE, a.RKAP_SUBPRO_CONTRACT_VALUE, TO_CHAR(a.RKAP_SUBPRO_CONTRACT_DATE, 'YYYY') AS RKAP_SUBPRO_CONTRACT_DATE, 
                b.RKAP_INVS_ID, b.RKAP_INVS_ASSETS, c.USER_ID, d.BRANCH_ID, d.BRANCH_NAME
                FROM TX_RKAP_SUB_PROGRAM a
                JOIN TX_RKAP_INVESTATION b ON a.RKAP_SUBPRO_INVS_ID = b.RKAP_INVS_ID
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND b.RKAP_INVS_ASSETS IN ('1', '3', '9', '7'))
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
        return $query->result();
    ...
    public function kontrak_peralatan_berjalan($get_tahun) {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_NILAI_CONTRACT FROM
            ...
            (
                ...
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND b.RKAP_INVS_ASSETS IN ('2', '4', '5', '6'))
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
        return $query->result();
    ...
    public function kontrak_non_fisik_berjalan($get_tahun) {
        ...
        $query =$this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.TOTAL_NILAI_CONTRACT FROM
            ...
            (
                ...
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND b.RKAP_INVS_ASSETS = 8)
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'
                GROUP BY BRANCH_ID, BRANCH_NAME
            ) B ON A.BRANCH_ID = B.BRANCH_ID");
        return $query->result();
    ...
    public function jumlah_kontrak_sipil_berjalan($get_tahun) {
        ...
        $query =$this->db->query("SELECT SUM(RKAP_SUBPRO_CONTRACT_VALUE) AS TOTAL_NILAI_CONTRACT
                FROM (SELECT a.RKAP_SUBPRO_INVS_ID, a.RKAP_SUBPRO_TITTLE, a.RKAP_SUBPRO_CONTRACT_VALUE, TO_CHAR(a.RKAP_SUBPRO_CONTRACT_DATE, 'YYYY') AS RKAP_SUBPRO_CONTRACT_DATE, 
                b.RKAP_INVS_ID, b.RKAP_INVS_ASSETS, c.USER_ID, d.BRANCH_ID, d.BRANCH_NAME
                FROM TX_RKAP_SUB_PROGRAM a
                JOIN TX_RKAP_INVESTATION b ON a.RKAP_SUBPRO_INVS_ID = b.RKAP_INVS_ID
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND b.RKAP_INVS_ASSETS IN ('1', '3', '9', '7') AND IS_PUSAT IN ('1', '0'))
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'");
        return $query->result();
    }
    ...
    public function jumlah_kontrak_peralatan_berjalan($get_tahun) {
        ...
        $query =$this->db->query("SELECT SUM(RKAP_SUBPRO_CONTRACT_VALUE) AS TOTAL_NILAI_CONTRACT
                ...
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND b.RKAP_INVS_ASSETS IN ('2', '4', '5', '6') AND IS_PUSAT IN ('1', '0'))
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'");
        return $query->result();
    }
    ...
    public function jumlah_kontrak_non_fisik_berjalan($get_tahun) {
        ...
        $query =$this->db->query("SELECT SUM(RKAP_SUBPRO_CONTRACT_VALUE) AS TOTAL_NILAI_CONTRACT
                ...
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND b.RKAP_INVS_ASSETS = 8 AND IS_PUSAT IN ('1', '0'))
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'");
        return $query->result();
    }
    ...
    public function jumlah_kontrak_total_report_berjalan($get_tahun) {
        $query = $this->db->query("SELECT SUM(RKAP_SUBPRO_CONTRACT_VALUE) AS TOTAL_NILAI_CONTRACT
                ...
                JOIN TM_USERS c ON b.RKAP_INVS_USER_ID = c.USER_ID
                JOIN TR_BRANCH d ON c.USER_BRANCH = d.BRANCH_ID
                WHERE a.IS_DELETED =0 AND b.IS_DELETED =0 AND IS_PUSAT IN ('1', '0'))
                WHERE RKAP_SUBPRO_CONTRACT_DATE = '$get_tahun'");
        return $query->result();
    }

    public function fisik_persentase_berjalan($get_bulan, $get_tahun) {
        $query = $this->db->query("SELECT A.BRANCH_ID, A.BRANCH_NAME, B.PERSENTASE FROM
        (
            SELECT * FROM TR_BRANCH
        ) A LEFT JOIN 
        (
            SELECT A.BRANCH_ID, A.BRANCH_NAME, (A.TOTAL_REALISASI / B.TOTAL_NILAI_CONTRACT * 100) AS PERSENTASE FROM
                ( 
                    SELECT BRANCH_ID, BRANCH_NAME, SUM(REAL_SUBPRO_VAL) AS TOTAL_REALISASI 
                    FROM (SELECT BRANCH_ID, BRANCH_NAME, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE 
                    FROM (SELECT RKAP_SUBPRO_ID, RKAP_SUBPRO_INVS_ID, SUM(REAL_SUBPRO_VAL) as REAL_SUBPRO_VAL, RKAP_INVS_VALUE, BRANCH_ID, BRANCH_NAME
