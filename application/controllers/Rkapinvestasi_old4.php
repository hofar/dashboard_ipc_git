...
class Rkapinvestasi extends CI_Controller {
    ...
    public function invest_project($page = 0) {
        // $data=array();
        $branch = $this->session->userdata('SESS_USER_BRANCH');
        $this->session->unset_userdata('search1');
        $this->session->unset_userdata('search2');
        ...
        if ($this->session->userdata('SESS_USER_PRIV') == 1) {
            ...
            $config['base_url'] = base_url() . 'rkapinvestasi/invest_project/';
            $config['total_rows'] = count($this->rkap_model->all());
            $this->pagination->initialize($config);
            $data['halaman'] = $this->pagination->create_links();
            $data['groups2'] = $this->rkap_model->all_investasi();
            $data['list'] = $this->rkap_model->all_limit1();
            $data['groups1'] = $this->rkap_model->all_aktiva();

            $this->load->view('template/global/header', $this->data);
            $this->load->view('template/pages/view_invest_project', $data);
            $this->load->view('template/global/footer');
        } else {
    ...
    public function investasi_new_ajax() {
        $result = $this->rkap_model->all();
        $data = array();
        foreach ($result as $row) {
            $data_row = array();

            $data_row[] = $row->RKAP_INVS_ID;
            $data_row[] = $row->RKAP_INVS_PROJECT_NUMBER;
            $data_row[] = $row->RKAP_INVS_TITLE;
            $data_row[] = $row->RKAP_INVS_YEAR;
            $data_row[] = $row->BRANCH_NAME;
            $invest_id = $this->rkap_model->all_investasi_byid($row->RKAP_INVS_TYPE)[0];
            $data_row[] = $invest_id->INVS_TYPE_NAME;


            $data[] = $data_row;
        };
        // $this->rkap_model->all_aktiva();

        echo json_encode(array('data' => $data));
    }

    public function po_kontrak() {
        $data = array();
        $this->load->view('template/global/header', $this->data);
        $this->load->view('template/pages/view_po_kontrak', $data);
        $this->load->view('template/global/footer');
    }

    public function list_subpro() {
        $this->load->view('template/global/header', $this->data);
        $this->load->view('template/pages/view_list_subpro', $data);
        $this->load->view('template/global/footer');
    }

    public function list_invest_a() {
        $this->load->view('template/global/header', $this->data);
        $this->load->view('template/pages/list_invest_a', $data);
        $this->load->view('template/global/footer');
    }

    public function ajax_list_po_kontrak() {
        $status = false;
        $data = array();
